  ...
  <!-- Facebook and Twitter integration -->
	...
	<div id="page">
		<nav class="colorlib-nav" role="navigation">
			...
			<div class="title" style="background-image: url(images/cars.jpg); width: 100%; height: 650px; background-size:cover ;"></div>
	...
	<div style=" font: 20px;">
            ...
            if(isset($_GET['from_city'])===true 
              ...
              if(empty($_GET['cartype'])===false ) {
				  
			  
			  
              echo '<h2>Cars with a type of  <a style="color: red;">'.$cartype.'</a>    </h2>';
              $query = "SELECT * FROM `carrental` WHERE `Rentloc`= '$from' AND `Returnloc` = '$to' AND `Cartype` ='$cartype'";
              $result = mysqli_query($conn, $query);
              if($result) {
              if(mysqli_num_rows($result) > 0) {
              while($row = mysqli_fetch_assoc($result)) {?>
			  <p>Available Car:</p><br>
                <table class="table" >
                  <thead>
                  <tr>
                    <th>Car No</th>
                  ...
                  <tbody>
                  <tr>
                   <td><input type="radio" name="chose_to2" value="<?php echo $row['CarRentalID']; ?>"/><?php echo $row['Brand']; ?></td>
                   <td><?php echo $row['Rentloc']; ?></td>				   
